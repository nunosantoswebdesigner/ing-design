...
import { Item, ItemAction, ItemContent, ItemDescription, ItemIcon, ItemLabel } from "@/components/ui/item";
...
export const ItemDemo = () => (
    <div className="w-full max-w-sm divide-y">
      ...
      <Item>
        ...
        <ItemContent><ItemLabel>Profile</ItemLabel></ItemContent>
      </Item>
      {/* Icon + label + description */}
      <Item>
        <ItemIcon><Mail /></ItemIcon>
        <ItemContent>
      ...
      <Item asChild>
        <button className="w-full cursor-pointer hover:bg-muted/50 rounded-md px-2 -mx-2 transition-colors">
          ...
          <ItemAction><ChevronRight className="size-4 text-muted-foreground" /></ItemAction>
        </button>
      </Item>
      {/* No icon */}
      <Item>
        ...
        <ItemAction><CreditCard className="size-4 text-muted-foreground" /></ItemAction>
      </Item>
    </div>
  );
